...
echo  \
"AUTOSSH_DEBUG=1 \
AUTOSSH_LOGFILE=/opt/lightup/lightup-ssh-connect.service.log \
AUTOSSH_PIDFILE=/opt/lightup/lightup-ssh-connect.service.pid \
autossh -f -M 0 ${LIGHTUP_CONNECT_USER_NAME}@${LIGHTUP_CONNECT_SERVER_NAME} -p ${LIGHTUP_CONNECT_SERVER_PORT} -N \
    ...
    -vvv -i /opt/lightup//keys/${LIGHTUP_CONNECT_KEYPAIR_NAME}" >> generated/lightup-ssh-connect-service.sh
